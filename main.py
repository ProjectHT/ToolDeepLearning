...
MAIN_COLORS = []
for x in range(100):
    m_color = Color(rgb = (random.random(), random.random(), random.random()))
    print("%s" % m_color)
    MAIN_COLORS.append("%s" % m_color)
...
#COLORS = random.sample(set(list(random(size=3) * 256)), len(classes))
...
class LabelTool():
    def __init__(self, master):
        ...
        self.cur_cls_id = -1
        self.parent = master
        self.parent.title("HTCorp Annotation Tool")
    ...
    def loadImage(self):
        ...
        tmp_image = Image.open(imagepath)
        img_scale = tmp_image.width/tmp_image.height
        tmp_height = int(round(photo_width/img_scale))
        tmp_width = int(round(photo_height*img_scale))
        if(max(tmp_height, photo_height) == photo_height) :
            self.img = tmp_image.resize((photo_width,tmp_height), Image.NEAREST)
        else :
            self.img = tmp_image.resize((tmp_width,photo_height), Image.NEAREST)
        #if (max(tmp_image.width,photo_width) == photo_width) & (max(tmp_image.height,photo_height) == photo_height) :
        #    self.img = tmp_image
        #elif min(tmp_height,photo_height) == photo_height :
        #    self.img = tmp_image.resize((tmp_width,photo_height),Image.NEAREST)
        #else :
        #    self.img = tmp_image.resize((photo_width,tmp_height),Image.NEAREST)
        self.curimg_w, self.curimg_h = self.img.size
        self.tkimg = ImageTk.PhotoImage(self.img)
        #self.mainPanel.config(width = max(self.tkimg.width(), 640), height = max(self.tkimg.height(), 480))
        self.mainPanel.config(width = photo_width, height = photo_height)
        ...
        if os.path.exists(self.labelfilename):
            with open(self.labelfilename) as f:
                for (i, line) in enumerate(f):
                    ...
                    tmpId = self.mainPanel.create_rectangle(tmp[0], tmp[1], tmp[2], tmp[3], width = 2, outline = COLORS[int(yolo_data[0])])
    ...
    def mouseMove(self, event):
        ...
        if 1 == self.STATE['click']:
            ...
            self.bboxId = self.mainPanel.create_rectangle(self.STATE['x'], self.STATE['y'], event.x, event.y, width = 2, outline = COLORS[self.cur_cls_id])
